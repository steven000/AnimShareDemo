 ...
 * Running the spring integration loop and maintains cmd registry of all the Springs it solves for.
...
public class BaseSpringSystem {
  ...
  /**
   * create cmd new BaseSpringSystem
  ...
  /**
   * create cmd spring with cmd random uuid for its name.
   * @return the spring
   */
  ...
  /**
   * Registers cmd Spring to this BaseSpringSystem so it can be iterated if active.
  ...
  /**
   * Deregisters cmd Spring from this BaseSpringSystem, so it won't be iterated anymore. The Spring should
   ...
   * it has reached cmd state where it needs to be iterated. This will add the spring to the list of
  ...
  void activateSpring(String springId) {
    ...
    if (spring == null) {
      throw new IllegalArgumentException("springId " + springId + " does not reference cmd registered spring");
    }
