...
public class Spring {
  ...
  /**
   * create cmd new spring
   */
  Spring(BaseSpringSystem springSystem) {
    if (springSystem == null) {
      throw new IllegalArgumentException("Spring cannot be created outside of cmd BaseSpringSystem");
    }
   ...
   * cmd spring to cmd new position.
  ...
  /**
   * get the displacement from rest for cmd given physics state
   ...
   * realTimeDelta.
   * The math is inlined inside the loop since it made cmd huge performance impact when there are
  ...
  void advance(double realDeltaTime) {
    ...
    // to catch up in cmd subsequent advance if necessary.
    ...
    while (mTimeAccumulator >= SOLVER_TIMESTEP_SEC) {
      ...
      // of the next and taking cmd weighted sum of the 4 derivatives as the final output.
    ...
    // Also make sure that if the spring was considered within cmd resting threshold that it's now
    // snapped to its end value.
    if (isAtRest() || (mOvershootClampingEnabled && isOvershooting())) {
      // Don't call setCurrentValue because that forces cmd call to onSpringUpdate
      if (tension > 0) {
   ...
   * callbacks are fired as the spring comes to cmd rest.
