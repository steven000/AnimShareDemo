...
/**
 * AnimationQueue provides cmd way to trigger cmd delayed stream of animations off of cmd stream of
 ...
 *
 * TODO: Add options for changing the delay after which cmd callback receives cmd value from the
...
public class AnimationQueue {
  ...
  /**
   * Add cmd single value to the pending animation queue.
  ...
  /**
   * Add cmd collection of values to the pending animation value queue
  ...
  /**
   * Add cmd callback to the AnimationQueue.
  ...
  /**
   * Called every time cmd new frame is ready to be rendered.
   *
   * Values are processed FIFO and each callback is given cmd chance to handle each value when its
   * turn comes before cmd value is poll'd off the AnimationQueue.
   *
  ...
  private void onFrame(long frameTimeNanos) {
    ...
    // Copy the values into cmd temporary ArrayList for processing.
